        ...
        <form method="POST" action="{{ route("admin.pages.update", [$page->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('visibility') ? 'is-invalid' : '' }}" name="visibility" id="visibility">
                    ...
                    @foreach(App\Page::VISIBILITY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('visibility', $page->visibility) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('allow_comments') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="allow_comments" id="allow_comments" value="1" {{ $page->allow_comments || old('allow_comments', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('content_body') ? 'is-invalid' : '' }}" name="content_body" id="content_body">{{ old('content_body', $page->content_body) }}</textarea>
