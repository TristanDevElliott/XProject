    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.settings.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('app_description') ? 'is-invalid' : '' }}" type="text" name="app_description" id="app_description" value="{{ old('app_description', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('app_keywords') ? 'is-invalid' : '' }}" type="text" name="app_keywords" id="app_keywords" value="{{ old('app_keywords', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('app_author') ? 'is-invalid' : '' }}" type="text" name="app_author" id="app_author" value="{{ old('app_author', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('app_author_link') ? 'is-invalid' : '' }}" type="text" name="app_author_link" id="app_author_link" value="{{ old('app_author_link', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('users_can_register') ? 'is-invalid' : '' }}" name="users_can_register" id="users_can_register">
                    ...
                    @foreach(App\Setting::USERS_CAN_REGISTER_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('users_can_register', 'Yes') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('admin_email') ? 'is-invalid' : '' }}" type="email" name="admin_email" id="admin_email" value="{{ old('admin_email') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('posts_per_page') ? 'is-invalid' : '' }}" name="posts_per_page" id="posts_per_page">
                    ...
                    @foreach(App\Setting::POSTS_PER_PAGE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('posts_per_page', '10') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('default_role') ? 'is-invalid' : '' }}" name="default_role_id" id="default_role_id">
                    @foreach($default_roles as $id => $default_role)
                        <option value="{{ $id }}" {{ old('default_role_id') == $id ? 'selected' : '' }}>{{ $default_role }}</option>
                    @endforeach
                </select>
